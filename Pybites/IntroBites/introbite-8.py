...
BeltStats = namedtuple('BeltStats', 'score ninjas') # convert a tuple to named tuple. BelStats is name of the tuple. score and ninjas are what is stored inside the
...
def get_total_points(belts=ninja_belts):
    ...
    total = 0
    for stat in belts.values(): #Use iteration variable for xxx in xxx
        total += stat.score * stat.ninjas # INside the for loop.
    return total
